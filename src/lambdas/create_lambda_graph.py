...
def main():
    ...
    script_file = os.path.join(current_dir, '../utility_scripts/create_graph.py')  # Tu script principal
    package_dir = 'lambda_package'         # Directorio para empaquetar el código
    zip_file = 'script_Graph.zip'               # Nombre del archivo ZIP
    script_key = 'script_Graph.zip'             # Clave del archivo en S3
    function_name = "LambdaScriptGraph"
    handler = "create_graph.main" # Handler de tu Lambda
...
#Invocar Lambda -> aws --endpoint-url=http://localhost:4566 lambda invoke --function-name LambdaScriptGraph output.json
#Mirar si el bucket existe -> aws --endpoint-url=http://localhost:4566 s3 ls
#Verificar los archivosde s3 -> aws --endpoint-url=http://localhost:4566 s3 ls s3://graph/
